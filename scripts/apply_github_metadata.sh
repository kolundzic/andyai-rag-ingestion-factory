...
# Topics are best-effort because gh versions differ.
if gh repo edit "$OWNER/$REPO_NAME" --help | grep -q -- "--add-topic"; then
  gh repo edit "$OWNER/$REPO_NAME" \
    --add-topic rag \
  ...
  echo "🟡 This gh version may not support --add-topic. Add topics manually if needed."
fi
